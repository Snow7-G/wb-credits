...
**[在 WorkBuddy 中打开技能页 →](workbuddy://skills)**

点上面的链接会弹出「打开 WorkBuddy？」，确认后客户端直接落到技能页——省去在侧边栏找入口。打开后切到 **套件** 标签，然后：

1. 点市场标签条右侧的 **＋** 按钮（鼠标悬停显示「添加市场」）
2. 「市场源」填 `Snow7-G/wb-credits`，提交
3. 找到 `wb-credits` 卡片，点它右侧的 **＋** 安装

> **链接没反应？** 手动走：左侧进入 **技能**（在「专家」区里），切到 **套件** 标签。插件不在一个叫「插件管理」的菜单里，这个名字容易找不到。
>
