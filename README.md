...
**验证环境**：WorkBuddy 桌面端 5.3.14（macOS）。数据读取依赖客户端内部存储，不保证其他版本一致——如果读不出来会明确报错，不会给错数字。

## 输出长这样

```
本对话                                         最近三轮
142.85                                         6.60｜7.47｜0.55
积分｜39 轮｜缓存命中 98%｜上下文 25 万        再聊 10 轮｜约 53 积分

未缓存输入 226 万 ｜ 缓存命中 8840 万 ｜ 输出 31 万
```

左栏是既成事实，右栏给你节奏感并往前推一步。缓存命中率单独占主数字下面那一行，因为它是解释"为什么花了这么多"的那个数字。

底栏只在有话说时出现，两种情况：单轮异常贵（附原因，如 `14:29 单轮 5.18（闲置 2h19m）`），或数据不完整（`用量明细有损坏，总数可能偏低`）。
