...
1. Open **Skills** from the sidebar (it lives under the Experts area), switch to the **Plugins** tab (套件)
2. Click the **＋** button to the right of the marketplace tabs — its tooltip reads "Add Marketplace" (添加市场)
3. Enter `Snow7-G/wb-credits` as the marketplace source and submit
4. Find the `wb-credits` card and click the **＋** on its right to install

> **Shortcut**: paste `workbuddy://skills` into your browser's address bar and hit Enter — it pops up "Open WorkBuddy?" and lands you directly on the Skills page. GitHub strips custom-protocol links from READMEs (they'd be unclickable), hence copy-paste.
>
> Plugins are not under a menu called "Plugin Management" — that name is easy to miss.
>
