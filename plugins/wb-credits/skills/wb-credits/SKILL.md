...
| 组合 | 结果 |
|---|---|
| `--all --format card` | 报错。排行没有卡片模板，用 `text` 或 `json` |
| `--no-tokens --estimate` | 报错。估算需要 token 数据 |
| `--limit` 负数 | 报错。负数在切片里是「从尾部截断」，会静默少给几行 |
...
**算错比报错糟糕。** 用户不会核对数字，他会直接信。时间戳拿不到时留空，不要猜一个填上。同理，`credit_json` 解析失败时总数会偏低，卡片底栏会标「用量明细有损坏，总数可能偏低」——这条提示不能省，省了用户就把偏低的数当完整的。

**读 trace 不要整份读。** `sessionId` 与 `modelInfo` 固定在文件开头（实测 571 个文件里最大偏移 318 / 387 字节），只读头 64KB 即可，全量扫描从约 1 秒降到约 42 毫秒。但 `modelInfo` 对象可能恰好跨过头部边界，所以必须做成「头部解析不出来就回头读整份」——只判断头部有没有那个字节串是不够的，实测能构造出整条 trace 被静默丢弃的样例。拆字段也要用括号配平取 `modelInfo` 对象，不能在全文跑正则：`spans` 里有同名的 token 字段。
